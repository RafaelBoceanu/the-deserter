...
public class WeaponHandler : MonoBehaviour
{
    ...
    public class Animations
    {
        ...
        public string realodingBool = "isReloading";
    ...
    void Animate()
    {
        if (!animator || currentWeapon.gameObject.activeInHierarchy == false)
            return;

        animator.SetBool(animations.aimingBool, aim);
        animator.SetBool(animations.realodingBool, reload);
    ...
    //Adds a weapon to the weaponList
    ...
    {
        if (settingWeapon || weaponsList.Count == 0 || reload)
            return;
