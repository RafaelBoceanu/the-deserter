...
public class CameraRig : MonoBehaviour
{
    ...
    //Follows the target with Time.deltaTime smoothly
    ...
    //Checks the wall and moves the camera up if wall hit
    ...
    void MoveCamUp(RaycastHit hit, Vector3 pivotPos, Vector3 dir, Transform cameraT)
    {
        ...
        Vector3 sphereCastCentre = pivotPos + (dir.normalized * hitDist);
        cameraT.position = sphereCastCentre;
    }

    //Positions the camera's local position to a given location
    ...
    //Hides the target's mesh renderers when too close
